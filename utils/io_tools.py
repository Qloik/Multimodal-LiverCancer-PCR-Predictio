...
def save_checkpoint(state,isUpdate, checkpoint_dir, filename='checkpoint.pth.tar',
    ...
    if isUpdate:
        ...
        torch.save(state, os.path.join(best_accuracy_path, filename))


def load_model(model, resume_dir, gpu, filename='model_best.pth.tar'):
    """
    ...
    param_dict = torch.load(os.path.join(resume_dir, filename), map_location="cpu")
...
def send_message(message):
    requests.post("http://192.144.232.194:15600/mvi_lxy", 
